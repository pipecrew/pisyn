// Package main demonstrates a cross-platform Go binary pipeline: lint, test, build, release.
// Works on both GitLab CI and GitHub Actions using platform-neutral variables.
package main
...
func main() {
	...
	pipeline := ps.NewPipeline(app, "Go CI CD").
		OnPush("main").
		OnPR("main").
		OnPushProtected().
	...
	goBase.Clone(build, "build-binary").
		Needs("gen-version").
		Script(
			"go build -ldflags \"-s -w -X main.version=" + genVersion.OutputRef("VERSION") + "\" -o bin/app ./cmd/app",
		).
		SetArtifacts(ps.Artifacts{
			Paths:    []string{"bin/"},
			ExpireIn: "30 days",
